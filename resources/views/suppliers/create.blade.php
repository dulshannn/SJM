...
@section('title', 'Add Supplier - Secure Jewellery Management')

@section('content')
<div style="max-width: 900px; margin: 0 auto;">
    <div style="margin-bottom: 30px;">
        <h1 class="gold-accent" style="font-size: 36px; margin-bottom: 8px;">
            <i class="fa-solid fa-truck-fast" style="margin-right: 12px;"></i>Add New Supplier
        </h1>
        <p style="color: #888; font-size: 14px;">Create a new supplier profile for jewellery deliveries</p>
    </div>
        ...
        <form action="{{ route('suppliers.store') }}" method="POST">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div>
                    ...
                    <input type="text" id="name" name="name" class="input-field" placeholder="Enter supplier name" required value="{{ old('name') }}">
                    @error('name')
            ...
            <div style="margin-bottom: 20px;">
                ...
                <input type="email" id="contact_email" name="contact_email" class="input-field" placeholder="Enter email address" required value="{{ old('contact_email') }}">
                @error('contact_email')
                    <small style="color: #ff9f9f; display: block; margin-top: 5px;">{{ $message }}</small>
                @enderror
            </div>
            ...
            <div style="margin-bottom: 25px;">
                ...
                <textarea id="address" name="address" class="input-field" placeholder="Enter supplier address" rows="3">{{ old('address') }}</textarea>
                @error('address')
                    <small style="color: #ff9f9f; display: block; margin-top: 5px;">{{ $message }}</small>
                @enderror
            </div>
            ...
            <div style="display: flex; gap: 15px;">
                <button type="submit" class="btn-gold" style="flex: 1;">
                    <i class="fa-solid fa-check" style="margin-right: 8px;"></i>Create Supplier
                </button>
